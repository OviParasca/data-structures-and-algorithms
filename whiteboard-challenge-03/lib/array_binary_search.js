...
var binarySearch = function(arr, val) {
    ...
    if(arr.length === 1) {
        ...
        return binarySearch(arr.sliceArray(half, arr.length), val);
    } else {
        return binarySearch(arr.sliceArray(0, half), val);
    }
};


Array.prototype.sliceArray = function(startingPoint, endingPoint) {
    const newArray = [];
